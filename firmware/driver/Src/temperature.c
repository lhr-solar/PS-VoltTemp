...
static ADC_ChannelConfTypeDef sConfig3 = {
    .Channel = TEMP3_ADC_CHANNEL,
    .Rank = ADC_REGULAR_RANK_1,
    .SamplingTime = TEMP_SAMPLE_TIME,
    .SingleDiff = ADC_SINGLE_ENDED,
    .OffsetNumber = ADC_OFFSET_NONE,
...
static ADC_ChannelConfTypeDef sConfig4 = {
    .Channel = TEMP4_ADC_CHANNEL,
    .Rank = ADC_REGULAR_RANK_1,
    .SamplingTime = TEMP_SAMPLE_TIME,
    .SingleDiff = ADC_SINGLE_ENDED,
    .OffsetNumber = ADC_OFFSET_NONE,
...
static ADC_ChannelConfTypeDef sConfig5 = {
    .Channel = TEMP5_ADC_CHANNEL,
    .Rank = ADC_REGULAR_RANK_1,
    .SamplingTime = TEMP_SAMPLE_TIME,
    .SingleDiff = ADC_SINGLE_ENDED,
    .OffsetNumber = ADC_OFFSET_NONE,
...
static int32_t ADCToTemp(uint16_t adc_val) {
    if (adc_val > MAX_ADC_COUNTS) {
        return thermistor_lut[MAX_ADC_COUNTS];
    }
...
temp_status_t Temp_StartSingleADC(bool clearQueue, thermistor_t temp_select) {
    ...
    {
    case TEMP1:
        if (adc_read(hadc1, &sConfig1, temp1_queue) != ADC_OK)
            return TEMP_ADC_START_FAIL;
        break;
    case TEMP2:
        if (adc_read(hadc1, &sConfig2, temp2_queue) != ADC_OK)
            return TEMP_ADC_START_FAIL;
        break;
    case TEMP3:
        if (adc_read(hadc1, &sConfig3, temp3_queue) != ADC_OK)
            return TEMP_ADC_START_FAIL;
        break;
    case TEMP4:
        if (adc_read(hadc1, &sConfig4, temp4_queue) != ADC_OK)
            return TEMP_ADC_START_FAIL;
        break;
    case TEMP5:
        if (adc_read(hadc1, &sConfig5, temp5_queue) != ADC_OK)
            return TEMP_ADC_START_FAIL;
        break;
    default:
        return TEMP_INVALID_CHANNEL;
...
temp_status_t Temp_GetSingleReading(thermistor_t temp_select, TempMsg_t* message, TickType_t ticksToWait) {
    ...
    if (message->raw_voltage > MAX_PLAUSIBLE_ADC_MV) {
...
temp_status_t Temp_GetAllReadings(TempMsg_t* messages, TickType_t ticksToWait) {
    temp_status_t status = TEMP_OK;

    // Read first 4 thermistors (only 4 on the segment, 5th is extra sooo)
    for (thermistor_t i = TEMP1; i < NUM_THERMISTORS - 1; i++) {
        status = Temp_GetSingleReading(i, &messages[i], ticksToWait);
    }
