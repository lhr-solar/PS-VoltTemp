...
//=================================================
BQ76920_Status bq76920_Read_1_Reg(uint16_t Mem_Address, uint8_t* read_Data, TickType_t delay_ticks){
  // check for valid pointer
  if(read_Data == NULL) return BQ_ERR;

  HAL_StatusTypeDef rx_status;

  // acquire mutex
  if(xSemaphoreTake(I2C_mutex, delay_ticks) != pdTRUE) return BQ_ERR;
...
uint8_t bq76920_R_1_bit(uint8_t reg, uint8_t bit_dec, TickType_t delay_ticks){
  if(reg >= END_OF_REGS)return BQ_ERR;
  uint8_t read_val; 
  if(bq76920_Read_1_Reg(reg, &read_val, delay_ticks) != BQ_OK) return 0xFF;
...
//=================================================
BQ76920_Status bq76920_Read(uint16_t Mem_Add_1, uint16_t Mem_Add_2, uint16_t* data, TickType_t delay_ticks)
{
  if(data == NULL) return BQ_ERR;
  uint8_t data_1; 
  if(bq76920_Read_1_Reg(Mem_Add_1, &data_1, delay_ticks) != BQ_OK) return BQ_ERR;
  uint8_t data_2;
  if(bq76920_Read_1_Reg(Mem_Add_2, &data_2, delay_ticks) != BQ_OK) return BQ_ERR;
...
static BQ76920_Status bq76920_Write(uint16_t Mem_Address, uint8_t new_data, TickType_t delay_ticks){
...
//==================================================================
BQ76920_Status bq76920_W_1_bit(uint8_t reg, uint8_t bit_dec, uint8_t state, TickType_t delay_ticks){
  uint8_t current; 
  if(bq76920_Read_1_Reg(reg, &current, delay_ticks) != BQ_OK) return BQ_ERR; // acquire current info
  ...
  // write to register.
  if(bq76920_Write(reg, new, delay_ticks) != BQ_OK) return BQ_ERR;
  return BQ_OK;
  //==================================================================
}
//==================================================================
...
uint8_t sys_Read(SysCommands Command, TickType_t delay_ticks)
{
...
// Details on datasheet (pgs. 29-32)
void sys_Write(SysCommands Command, uint8_t State, TickType_t delay_ticks)
{
  // Depending on which SYS command you want, a different register may be used.
...
uint8_t protect_Read(ProtectCommands Command, TickType_t delay_ticks)
{
  // Return bit corresponding to command.
...
// Inputs are which "Command" to adjust and what to set the bit to.
// Details on datasheet (pgs. 32-35)
void protect_Write(ProtectCommands Command, uint8_t State, TickType_t delay_ticks)
{
  // Depending on which PROTECT command you want, a different register may be used.
  // Write to the correct "command"
  switch (Command)
...
//====================================================================
uint32_t get_Voltage_1(uint16_t cell, TickType_t delay_ticks)
{
  // Cell is passed in as 0xabcd, this reads 0xab and 0xcd
  uint16_t ADC_reading; bq76920_Read((cell >> 8), (cell & 0xFF), &ADC_reading, delay_ticks);
...
//====================================================================
BQ76920_Status get_Voltage_All(uint32_t *voltage_array, TickType_t delay_ticks)
{
  // make sure valid pointer
  if(voltage_array == NULL) return BQ_ERR;

  voltage_array[0] = get_Voltage_1(VC1, delay_ticks);
  voltage_array[1] = get_Voltage_1(VC2, delay_ticks);
  voltage_array[2] = get_Voltage_1(VC3, delay_ticks);
  // VC4 is skipped, as it is the shorted cell for our application
  voltage_array[3] = get_Voltage_1(VC5, delay_ticks);

  voltage_array[4] = (4 * get_Voltage_1(BAT, delay_ticks));
